...
MEMORY_SECTION
REAL second() {
	return (REAL)msCounter / (REAL)1000.0;
}
 ...
 ************************************************************************/
    ...
    printf( "----------------- ----------------- --------- "
                      "--------- ---------\n");
    printf( "LINPACK BENCHMARK FOR PCs 'C/C++'    n @ 100\n\n");
    printf( "Month run         %d/%d\n", this_month, this_year);
    printf( "Compiler          %s\n", compiler);
    printf( "OptLevel          %s\n", options);
    
    printf("Rolling            %s\n",ROLLING);
    printf("Precision          %s\n",PREC); 
    printf("norm. resid        %16.1f\n",(double)residn);
    printf("resid              %16.8e\n",(double)resid);
    printf("machep             %16.8e\n",(double)epsn);
    printf("x[0]-1             %16.8e\n",(double)x1);
    printf("x[n-1]-1           %16.8e\n",(double)x2);
    printf("matgen 1 seconds   %16.5f\n",overhead1);
    printf("matgen 2 seconds   %16.5f\n",overhead2); 
    printf("Repetitions        %16d\n",ntimes);
    printf("Leading dimension  %16d\n",lda);  
    printf("                              dgefa     dgesl "
                     "    total    Mflops\n");
    printf("1 pass seconds     %16.5f %9.5f %9.5f\n",
                      atime[0][0], atime[1][0], atime[2][0]);
                      
    for (i=1 ; i<6 ; i++)
    {                 
        printf("Repeat seconds     %16.5f %9.5f %9.5f %9.2f\n",                
                       atime[0][i], atime[1][i], atime[2][i], atime[3][i]);
    }
    printf("Average            %46.2f\n",atime[3][6]);
